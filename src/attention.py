...
def dual_attention_shared_decoder(decoder_inputs, initial_state, encoder_states, dec_2_decoder_states, enc_padding_mask, \
                      cell, dec_2_padding_mask, initial_state_attention=False, reuse=False,
                      use_dual=False, pad_last_state=False):
    if reuse:
        reuse = tf.AUTO_REUSE
    ...
    with variable_scope.variable_scope("dual_attention_decoder", reuse=reuse) as scope:
        ...
        if use_dual:
            dual_shared_params = flat_attention_init(dec_2_decoder_states, dec_2_padding_mask, use_dual=True,
                                                    scope="Dual_Attention")

        def stop_when(time, unused_state, unused_last_states, unused_context_vector, unused_dual_context_vector, unused_decoder_states,
                      unused_attn_dists, unused_dual_attn_dists, unused_outputs):
            return tf.less(time, tf.constant(len(decoder_inputs)))

        def one_step(time, state, last_states, context_vector, dual_context_vector, decoder_states, attn_dists, dual_attn_dists,
                     outputs):
        ...
        initial_context_vector.set_shape([None, attn_size])  # Ensure the second shape of attention vectors is set.
        initial_dual_context_vector = array_ops.zeros([batch_size, dual_attn_size])
        initial_dual_context_vector.set_shape(
            [None, dual_attn_size])  # Ensure the second shape of attention vectors is set.
        ...
        initial_context_vector = tf.cond(initial_state_attention, \
                                                           lambda: (vec), \
                                                           lambda: (initial_context_vector))  # array_ops.zeros([batch_size, attn_size]),
        if use_dual:
        ...
        time, last_state, last_states, context_vector, dual_context_vector, decoder_states, attn_dists, dual_attn_dists, outputs = \
            tf.while_loop(stop_when, one_step, \
                          loop_vars=[initial_time, initial_state, initial_last_states, initial_context_vector, initial_dual_context_vector,
        ...
        if pad_last_state:
            ...
            # last_state_0 = [tf.unstack(x, axis=2)[ind] for x, ind in zip(tf.unstack(last_states, axis=3), tf.unstack(padding_inds))]
            ...
            # last_state_1 = [x[ind] for x, ind in zip([tf.unstack(last_states, axis=3)[1]], tf.unstack(padding_inds))]
...
def dual_attention_decoder(decoder_inputs, initial_state, encoder_states, dec_2_decoder_states, enc_padding_mask, \
                      cell, dec_2_padding_mask, initial_state_attention=False, reuse=False,
                      use_dual=False):
    with variable_scope.variable_scope("dual_attention_decoder", reuse=reuse) as scope:
        batch_size = encoder_states.get_shape()[0].value  # if this line fails, it's because the batch size isn't defined
        ...
        def stop_when(time, unused_state, unused_context_vector, unused_dual_context_vector, unused_decoder_states,
                      unused_attn_dists, unused_dual_attn_dists, unused_outputs):
            return tf.less(time, tf.constant(len(decoder_inputs)))
...
def flat_attention_init(encoder_states, enc_padding_mask, use_dual=False, reuse=False, scope=None):
    with variable_scope.variable_scope(scope or "Attention"):
        ...
        return (batch_size, attn_size, encoder_states, attention_vec_size, encoder_features, v, enc_padding_mask, w_c)


def flat_attention(decoder_state, shared_params, scope=None):
    (batch_size, attn_size, encoder_states, attention_vec_size, encoder_features, v, enc_padding_mask,
    w_c) = shared_params
    ...
    with variable_scope.variable_scope(scope or "Attention"):
        ...
        # Calculate v^T tanh(W_h h_i + W_s s_t + b_attn)
        e = math_ops.reduce_sum(v * math_ops.tanh(encoder_features + decoder_features), [-1, -2])  # calculate e
